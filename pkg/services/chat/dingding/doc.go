...
//
//	dingding://<access_token like abcdef123...>?[secret=<secret like SECabcdef123...>][&keyword=<keyword>][&title=<title>][&template=<template>]
//
...
//
//	dingding://<client_id>?secret=<client_secret>&userids=<comma splited userIDs>&[title=<title>][&template=<template>]
//
// To use worknotice mode, you need to create an "Internal Application" in Dingding's developer console, and get the "Client ID" and "Client Secret" for the application.
// Create a bot for it and release it (this is mandatory, otherwise the API will return "robotCode not found").
